...
export default function ScrollMorphHero({ 
    images, 
    title = "Our Memories",
    subtitle = "Scroll to explore our journey together"
    ...
    // Morph Progress: Keep at 0 to stay in circle (no morphing to arc)
    const morphProgress = useMotionValue(0);
    const smoothMorph = useSpring(morphProgress, { stiffness: 40, damping: 20 });

    // No rotation - keep circle stable
    const smoothScrollRotate = useMotionValue(0);
    ...
    return (
        <div ref={containerRef} className="relative w-full h-full bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 overflow-hidden">
                ...
                <div className="relative flex items-center justify-center w-full h-full">
                    {images.slice(0, TOTAL_IMAGES).map((src, i) => {
                        ...
                        if (introPhase === "scatter") {
                            ...
                            // A. Calculate Circle Position - Responsive to screen size
                            let circleRadius: number;
                            
                            if (isMobile) {
                                // Mobile: Use 35% of smaller dimension, max 280px
                                circleRadius = Math.min(minDimension * 0.35, 280);
                            } else if (isTablet) {
                                // Tablet: Use 40% of smaller dimension, max 400px
                                circleRadius = Math.min(minDimension * 0.4, 400);
                            } else {
                                // Desktop: Use 45% of smaller dimension, max 500px
                                circleRadius = Math.min(minDimension * 0.45, 500);
                            }
                            
                            // Fixed positions - no rotation
                            const baseCircleAngle = (i / TOTAL_IMAGES) * 360;
                            const circleRad = (baseCircleAngle * Math.PI) / 180;
                            const circlePos = {
                                x: Math.cos(circleRad) * circleRadius,
                                y: Math.sin(circleRad) * circleRadius,
                                rotation: baseCircleAngle + 90, // Keep images upright, fixed rotation
                            };

                            // Stay in circle - no morphing to arc
                            target = {
                                x: circlePos.x,
                                y: circlePos.y,
                                rotation: circlePos.rotation,
                                scale: 1,
                                opacity: 1,
                            };
                        }
